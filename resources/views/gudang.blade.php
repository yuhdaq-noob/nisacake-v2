            ...
            <div class="bg-white shadow-card rounded-2xl border border-slate-100 overflow-hidden flex flex-col">
                <div class="px-4 sm:px-5 py-3 sm:py-4 border-b border-slate-100 sticky top-0 bg-white z-10">
                    <div class="flex items-center justify-between gap-3">
                        ...
                        <span class="text-xs font-medium px-2.5 py-1 rounded-full bg-cyan-50 text-cyan-700"
                        >Live</span>
                    </div>
                </div>
        ...
        <div class="relative z-10 flex items-center justify-center min-h-screen p-4">
            <div class="modal-panel w-full max-w-lg bg-white rounded-2xl shadow-lg border border-slate-100 transform transition-all duration-200 ease-out translate-y-4 scale-95 opacity-0">
                <div class="flex items-center justify-between px-4 sm:px-5 py-3 sm:py-4 border-b border-slate-100">
                    <div>
                        <p class="text-sm font-semibold text-slate-900 flex items-center gap-2"><i class="bi bi-cart-plus-fill text-cyan-600"></i> Input Belanja Bahan</p>
                ...
                <form id="formRestock" data-ajax="true" class="px-4 sm:px-5 py-4 space-y-4">
                    <div class="space-y-2">
                        ...
                        <div id="error_restock_material" class="hidden text-xs text-red-600 mt-1"></div>
                    </div>
                    <div class="space-y-2">
                        ...
                        <div id="error_restock_amount" class="hidden text-xs text-red-600 mt-1"></div>
                    </div>
                    <div class="space-y-2">
                        ...
                        <div id="error_restock_description" class="hidden text-xs text-red-600 mt-1"></div>
                    </div>
    ...
    <div id="modalKurangStok" class="modal fixed inset-0 z-50 hidden opacity-0 pointer-events-none transition-opacity duration-200 ease-out">
        ...
        <div class="relative z-10 flex items-center justify-center min-h-screen p-4">
            <div class="modal-panel w-full max-w-lg bg-white rounded-2xl shadow-lg border border-slate-100 transform transition-all duration-200 ease-out translate-y-4 scale-95 opacity-0">
                <div class="flex items-center justify-between px-4 sm:px-5 py-3 sm:py-4 border-b border-slate-100">
                    <div>
                        <p class="text-sm font-semibold text-red-700 flex items-center gap-2"><i class="bi bi-exclamation-triangle-fill text-red-500"></i> Catat Pengurangan Stok</p>
                ...
                <form id="formKurangStok" data-ajax="true" class="px-4 sm:px-5 py-4 space-y-4">
                    ...
                    <div class="space-y-2">
                        <label class="text-sm font-semibold text-slate-900">Pilih Bahan Baku</label>
                        <select id="selectKurang" name="material_id" class="w-full rounded-lg border border-slate-200 bg-slate-50 text-slate-900 px-3 py-2 focus:bg-white focus:border-cyan-500 focus:ring-2 focus:ring-cyan-200/50 transition-colors" required>
                            <option value="">-- Pilih Bahan --</option>
                            @foreach($materials as $m)
                                <option value="{{ $m->id }}">{{ $m->name }} (Sisa: {{ $m->current_stock }} {{ $m->unit }})</option>
                            @endforeach
                        </select>
                        <div id="kurangStockInfo" class="text-xs text-slate-500 mt-1">Sisa: —</div>
                        <div id="error_kurang_material" class="hidden text-xs text-red-600 mt-1"></div>
                    </div>
                    <div class="space-y-2">
                        <label class="text-sm font-semibold text-slate-900">Jumlah Berkurang</label>
                        <input id="kurangAmount" type="number" name="amount" class="w-full rounded-lg border border-slate-200 bg-slate-50 text-slate-900 px-3 py-2 focus:bg-white focus:border-cyan-500 focus:ring-2 focus:ring-cyan-200/50 transition-colors" min="1" placeholder="Contoh: 5" required>
                        <p class="text-xs text-slate-500 mt-1">Masukkan angka sesuai satuan bahan.</p>
                        <div id="error_kurang_amount" class="hidden text-xs text-red-600 mt-1"></div>
                    </div>
                    <div class="space-y-2">
                        <label class="text-sm font-semibold text-slate-900">Keterangan / Alasan</label>
                        <textarea id="kurangDescription" name="description" class="w-full rounded-lg border border-slate-200 bg-slate-50 text-slate-900 px-3 py-2 focus:bg-white focus:border-cyan-500 focus:ring-2 focus:ring-cyan-200/50 transition-colors" rows="3" placeholder="Wajib diisi! Contoh: Telur pecah atau tepung basah." required></textarea>
                        <p class="text-xs text-slate-500 mt-1">Jelaskan alasan secara singkat (maks 255 karakter).</p>
                        <div id="error_kurang_description" class="hidden text-xs text-red-600 mt-1"></div>
                    </div>
                    <div class="flex flex-col-reverse sm:flex-row justify-end gap-2 pt-2">
